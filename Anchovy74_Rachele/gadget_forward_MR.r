#Usado para generic_DLM_code_Marga.R
effort=seq(0.1,1.5,by = 0.05);years = 100; params.file = "WGTS/params.final"; main.file = "main"; 
    num.trials = 10; fleets = pre.fleet[3,];#data.frame(fleet = "seine", ratio = 0.5); 
    biomass = FALSE;  spawnmodel = "none"; spawnvar = NULL; 
    selectedstocks = NULL; biomasslevel = NULL; check.previous = FALSE; 
    save.results = TRUE; stochastic = TRUE; rec.window =c(2001,2015); 
...
#}
  ...
  #  } else {
    ...
    # } else {
        ...
        #este no lo pilla bien
        for (i in 1:years) {
            rec.forward[, i + 1] <- coeffAR[2] * rec.forward[,i] + x[,i]
        }
        rec.out <- arrange(melt(rec.forward[, -1], value.name = "recruitment"),trial,year)
...
#
    ...
    #check anch file y cambiar normalparam por normalcond
    callGadget(s = 1, i = sprintf("%s/params.forward", pre), 
        main = sprintf("%s/main.pre", pre), log=sprintf("%s/tmp.pre", pre))
