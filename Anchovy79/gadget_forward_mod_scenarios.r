...
#To put seasonal recruitment is only necessary to add collapse=FALSE in get.gadget.recruitment and add the step in arrange
	...
	for (i in 1:steps) {
	rec.forward.steps.fv[, i + 4]<-coeffAR[1]+coeffAR[2]*rec.forward.steps.fv[,i]+coeffAR[3]*Windvalues[i]+coeffAR[4]*Windvalues[i+1]
	...
	for (i in 1:years) {
            rec.forward[,i+1] <- rowSums(rec.forward.steps.fv[,(i*4+1):(i*4+4)])+y[,i]
        }
	...
	save(list = ls(envir = environment(), all.names = TRUE),
     file = sprintf("%s/preout.Rdata", pre), envir = environment())
